...
| session-memory hook（/new / /reset） | ✅ 写 workspace/memory/ | ❌（before_reset hook 拦截）|
| 读租户自身历史画像 | ✅ | ❌（存储在插件 data/ 下，不在 workspace/）|
| 跨租户读取 | ✅ | ❌（agentId 隔离）|

#### 3.4.2 实现机制

租户的 `memorySearch.enabled: true`，允许通过 `memory_search` / `memory_get` 只读查询全局记忆。

写入拦截点：
- `before_tool_call`：拦截 `write` / `edit` → 返回 `{ block: true, blockReason: "无写入权限" }`
- `before_reset`：检查 agentId，若非 main 则阻止触发 session-memory hook
...
Slug 由 LLM（使用 Owner 的 auth profile）基于会话最近 10 条消息生成，调用不计入租户配额。
...
- LLM 生成 slug 使用 `generateSlugViaLLM`（来自 `openclaw/dist/bundled/session-memory/handler.js`），不暴露到 Plugin SDK，需动态 import
- `before_reset` hook 是否能阻止 bundled session-memory hook 触发，需测试验证（见 TESTING.md）
